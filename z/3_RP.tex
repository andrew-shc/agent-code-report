...
Forward pass: PathTracer 16~spp. Backward: ReSIR 1~spp with unbiased
balance-heuristic (Algorithm~6). WandB project: ReSTIR PBIR.

\begin{table}[ht]
\centering
\caption{Cornell box shape optimization (50~iters).}
\label{tab:cornell}
\footnotesize
\begin{tabular}{@{}lcccccS[round-precision=2]@{}}
\toprule
Method &
  \makebox[0.8cm][c]{$n_{\text{cand}}$} &
  \makebox[0.8cm][c]{bwd~SPP} &
  \makebox[0.6cm][c]{Unbias} &
  Final~L1 &
  \makebox[0.9cm][c]{Y (GT=82.5)} &
  {it/s} \\
\midrule
Standard PT & --- & 16 & --- & 0.00662 & 81.3 & 0.210 \\
ReSIR cand4 & 4   & 1  & \checkmark & 0.00667 & 87.4 & 0.124 \\
ReSIR cand8 & 8   & 1  & \checkmark & 0.00662 & 86.5 & 0.124 \\
\bottomrule
\end{tabular}
\end{table}

Runs: \texttt{standard} (9k6cyex9), \texttt{cand4~unbiased} (sbgbjs30),
\texttt{cand8~unbiased} (euoh8as1). The standard PathTracer is the
\texttt{backward\_mode='path'} ablation (no ReSIR gradient). All forward
passes use PathTracer; ReSIR replaces only the backward gradient. The~Y
values show remaining optimization error consistent across methods at
similar L1 loss --- ReSIR converges to comparable quality at 64$\times$ fewer
backward samples (1 vs.\ 16 spp).

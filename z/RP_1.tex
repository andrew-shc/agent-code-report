\title{Report 1: Neural-PBIR with PathReSTIR Backward Gradient}
...
resampling: candidates paths are reused across neighboring pixels to reduce
...
Figure~\ref{lst:renderer} shows the core Python autograd wrapper.
...
\texttt{renderD()} for the spatial-resampling gradient. The
\texttt{PSDRJITConnector} is patched via
...
Table~\ref{tab:results} presents the per-scene and aggregate metrics.
Key observations:
...
PathReSTIR provides a 2$\times$ speed-up in the PBIR stage while preserving
shape, geometry, and material quality. The gap in visual metrics (NVS and
relighting) is attributable to the reduced forward-pass sample count rather than
the backward-pass algorithm. Promising directions for closing this gap include:

\begin{itemize}
    \item Increasing the number of spatial candidates from 8 to 16 or 32.
    \item Increasing the forward-pass spp from 16 to 32 (still a ray saving
    vs the 64-spp standard).
    \item Tuning the spatial resampling radius and neighbor count per scene.
    \item Applying temporal reuse (extending ReSTIR to ReSTIR DI) for the
    backward gradient pass.
\end{itemize}

\begin{acks}
The authors thank the Neural-PBIR and psdr-jit teams for their open-source
implementations, which made this work possible.
\end{acks}

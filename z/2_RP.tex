...
\title{Report 2: ReSIR Verification \& Architecture --- Live Working Document}
...
in \texttt{GREENFIELD/RESTIR.md} \S5). The bias is localized to
...
The decoupling is visible in the Python wrapper (\texttt{run\_restir.py:168--199}):
...
architecture block comment at \texttt{run\_restir.py:33--65}.
...
\emph{File:} \texttt{run\_restir.py:119--154}

Two patches are applied at import time, before any scene is constructed:

\begin{description}
    \item[\texttt{Mesh} handler patch (lines~125--135):] Re-applies
    ...
    \item[\texttt{Integrator} handler patch (lines~139--153):] Registers the
...
\emph{File:} \texttt{run\_restir.py:202--224}
...
\emph{File:} \texttt{run\_restir.py:256--258}
...
There \textbf{is} an external library that does differentiable ReSTIR with a
backward pass, but it uses a completely different technology stack (SlangPy
vs.\ Dr.Jit). Our psdr-jit approach is architecturally valid, independently
correct, and more appropriate for integration with the existing
Neural-PBIR pipeline. MIRReS could serve as a mathematical verification
reference for specific numerical values.
...
\begin{itemize}
    ...
    \item Architecture block comment in \texttt{run\_restir.py} (lines~58--64).
...
\begin{itemize}
    ...
    \item[\,] \textbf{Verify ReSIR mathematical correctness} against the
    ...
    \item[\,] \textbf{Investigate why forward spp has minimal impact} --- is
...
\begin{itemize}
    ...
    \item \textbf{test\_autograd.py} (21/22 passing, 1 xfail): CUDA
...
\texttt{PathReSTIRFunction} & \texttt{run\_restir.py:168--199} & \texttt{torch.autograd.Function} \\
\texttt{PathReSTIRRenderer} & \texttt{run\_restir.py:202--224} & \texttt{torch.nn.Module} wrapper \\
\texttt{PSDRJITConnector patches} & \texttt{run\_restir.py:119--154} & \texttt{use\_face\_normal} + integrator registration \\
\texttt{PathReSTIR header} & \texttt{path\_restir.h:50--54} & Reservoir member variable declarations \\
ReSIR domain knowledge & \texttt{GREENFIELD/RESTIR.md} & Algorithm reference \\
...
\texttt{test\_autograd.py} & \texttt{GREENFIELD/tests/} & CUDA integration tests, 21/22 pass (1 xfail) \\
